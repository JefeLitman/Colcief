													...
													@foreach ($recuperacion[$p->pk_periodo] as $r)
														<tr>	
															...
															<td class="text-center">
																...
																@if (strtotime(date('d-m-Y'))>=strtotime($p->recuperacion_inicio) and strtotime(date('d-m-Y'))<=strtotime($p->recuperacion_limite))
																	<a href="/recuperaciones/{{$r->pk_recuperacion}}/editar" data-toggle="tooltip" data-placement="right" title="Editar"><i class="fas fa-edit text-info"></i></a>
																@else
																	<a href="" data-toggle="tooltip" data-placement="right" title="Solo puede ser editado desde el {{$p->recuperacion_inicio}} hasta las 23:59 del {{$p->recuperacion_limite}}."><i class="fas fa-edit  text-secondary"></i></a>
																@endif
															</td>
														</tr>
													@endforeach
